...
- ~~Phases 8–13 are untouched.~~ **Phase 8 (hints) is built (§6.41). Phase 9 (save-data versioning, per-mechanic skill, hint telemetry) is started (§6.42) — the recording exists, nothing consumes it yet.** Phases 10–13 remain: daily challenge, campaign/world UI, mobile polish, QA gate. The pack-select and mode-select UI (`SetPack`/`SetMode` exist and nothing calls them) is still the one blocking the packs being reachable in play at all — it and Phase 9 were the only two candidates with no product decision gating them; Phase 9 was picked this round, pack-select UI is the natural one after it.
